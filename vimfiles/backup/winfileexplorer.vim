...
" Contributors were: Doug Potts, Bram Moolenaar, Thomas K枚hler
"
...
function! s:ExtractFileDate(line)
	if w:longlist==0
		...
		return strpart(matchstr(strpart(a:line,b:maxFileLen+b:maxFileSizeLen+4),"芦.*"),1) + 0
	endif
endfunction
...
function! s:FileModDate(name)
	...
	if filetime > 0
		return strftime(g:explDateFormat,filetime) . " 芦" . filetime
	else
		return ""
	endif
endfunction
